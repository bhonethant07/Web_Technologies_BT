...
const DashboardHeader = ({ user, stats, className = "" }) => {
  // If data is not yet loaded
  if (!user || !stats) {
    return (
      <div className={`bg-white bg-opacity-80 backdrop-blur-sm border border-gray-200 rounded-xl p-6 shadow-md ${className}`}>
  ...
  return (
    <motion.div
      ...
      className={`bg-white bg-opacity-80 backdrop-blur-sm border border-gray-200 rounded-xl p-6 shadow-md ${className}`}
    >
